/// Observer Daemon for X11 Window Focus Tracking
///
/// This is a separate binary that runs as a daemon process to monitor
/// X11 window focus events and communicate them to the main Ronin application
/// via Unix socket IPC.
///
...
async fn run_daemon() -> Result<(), Box<dyn std::error::Error>> {
    eprintln!("[observer] Observer daemon is only supported on Linux/X11");
