...
- **THE GAP:** No code connects Vercel AI SDK `generateText` → calls tools → emits events → updates UI
...
**Exit Point:** ✅ Tool calls appear in reasoningStore.stepHistory, ThinkingIndicator updates
...
- [ ] Verify `src/lib/ai/tools/wrapper.ts` calls `logToolCall` after execution (line ~298)
- [ ] Verify `logToolCall` in `logger.ts` appends to `reasoningStore.stepHistory`:
...
**Exit Point:** ✅ Hook calls AI with tools, tools execute, store updates
...
**Exit Point:** ✅ Full UI updates live, ProtocolViewer/ThinkingIndicator work
...
**Exit Point:** ✅ Full E2E flow works, tests pass

- [ ] Manual E2E test:
  - [ ] Dashboard → Expand card → "Deeper Analysis" → Agent View
  - [ ] Mode toggle to "Thinking"
  - [ ] Click "Analyze Project"
  - [ ] Verify:
    - ThinkingIndicator shows tool calls
    - ProtocolViewer steps progress from pending → active → done
...
| Anthropic | 401 Unauthorized | "API key invalid. Check Settings → AI Provider." |
...
const TOOL_DISPLAY_MAP: Record<string, { icon: string; verb: string }> = {
  ...
  'list_dir': { icon: '📁', verb: 'Scanning' },
  'git_status': { icon: '🔀', verb: 'Checking git status' },
  'git_log': { icon: '📜', verb: 'Analyzing commit history' },
};

// Usage: "📖 Reading package.json..."
function formatToolCall(toolCall: string): string {
  ...
  const display = TOOL_DISPLAY_MAP[toolName] || { icon: '🔧', verb: 'Using' };
...
2. **Integration Tests (Manual/E2E):**
   - Full flow: Dashboard → Agent View → Analyze → See Results
...
- **勇 (Yu) - Courage:** Agent takes initiative but doesn't command - "Suggested next steps" not "You must do"
- **仁 (Jin) - Compassion:** Error messages are empathetic ("Unable to read file" not "File read failed")
- **智 (Chi) - Resourcefulness:** Efficient context payload (\<10KB), works on modest hardware
...
**Expected Result:**
- ThinkingIndicator shows: "📖 Reading package.json...", "🔍 Analyzing git history...", "📝 Checking DEVLOG..."
- ProtocolViewer steps progress: 1 (done) → 2 (active) → 3 (active) → ... → 5 (done)
- Final report appears in AgentChat with markdown formatting
- Attribution shows: "Based on: 📁 3 files · 🔀 20 commits"
...
**Expected Result:**
- ThinkingIndicator shows: "📖 Reading package.json..." then "⚠️ File not found"
...
#    - "[useAgentExecution] Status changed: idle → analyzing"

# 3. Manual test flow (can't automate Tauri easily):
#    Dashboard → Expand "ronin" card → Click "🧠 Deeper Analysis"
#    → Verify URL is /agent/1 → Click "Analyze Project"
#    → Watch ThinkingIndicator and ProtocolViewer update
#    → Wait for final report with "Based on:" attribution
...
<!-- Dev agent records completion notes here -->
...
**Files to Modify:**
- `src/lib/ai/tools/wrapper.ts` (Add event emission - lines ~46-47)
- `src/components/agent/AgentChat.tsx` (Replace useAgentAnalysis with useAgentExecution)

**Files to Review (Context):**
- `src/stores/reasoningStore.ts` (Understand state structure)
- `src/lib/ai/prompts/ronin-thinking.ts` (System prompt for generateText)
- `src/lib/ai/registry.ts` (Tools registry)
- `src/hooks/useAgentAnalysis.ts` (Existing pattern, might deprecate)

## Change Log

**2025-12-23:** Story 4.5.4 created via BMad create-story workflow
- Comprehensive context from Stories 4.5.1, 4.5.2, 4.5.3
- Integration design based on Sprint Change Proposal
- Sub-tasks with clear exit points (~6h total estimated)
- Ready for dev-story workflow

## Story Completion Status

- [ ] Sub-task 1: Tool wrapper emits events
- [ ] Sub-task 2: useAgentExecution hook created
- [ ] Sub-task 3: AgentChat integration complete
- [ ] Sub-task 4: E2E testing passed
