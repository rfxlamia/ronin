...
- [ ] Task 1: Create Observer Daemon Binary Structure
  - [ ] 1.1: Add `src-tauri/src/bin/observer.rs` (Cargo binary target)
  - [ ] 1.2: Add dependencies: `x11rb-async`, `tokio` (full), `serde_json`
  - [ ] 1.3: Implement Unix socket listener/client for IPC

- [ ] Task 2: Implement Async X11 Monitoring (AC: #3)
  - [ ] 2.1: Connect using `x11rb_async::rust_connection::RustConnection`
  - [ ] 2.2: Subscribe to `PropertyChange` events on root window
  - [ ] 2.3: Implement async property fetching for `_NET_ACTIVE_WINDOW`, `_NET_WM_NAME`, `WM_CLASS`

- [ ] Task 3: Implement Debouncing & IPC (AC: #2, #4)
  - [ ] 3.1: Implement 500ms debounce loop
  - [ ] 3.2: Serialize `WindowEvent` to JSON
  - [ ] 3.3: Send data to main app via Unix socket (`/tmp/ronin-observer.sock`)

### Part B: Main App Integration
- [ ] Task 4: IPC Server & Process Management (AC: #1, #6, #7)
  - [ ] 4.1: Create `ObserverManager` in `src-tauri/src/observer/mod.rs`
  - [ ] 4.2: Implement `start_daemon` (spawn process) and `stop_daemon` (kill/signal)
  - [ ] 4.3: Implement Unix socket server to receive daemon events

- [ ] Task 5: Data Persistence & Project Matching (AC: #5)
  - [ ] 5.1: Listen for IPC messages in background task
  - [ ] 5.2: Match window titles to project paths
  - [ ] 5.3: Batch insert into `observer_events` table (migration 003)

- [ ] Task 6: Tauri Commands & State
  - [ ] 6.1: Add `ObserverManager` to `AppState`
  - [ ] 6.2: Implement `start/stop` commands linked to settings
  - [ ] 6.3: Ensure daemon is killed on app exit
