...
- Backend currently supports only single `system_prompt + user_message` → This story adds `messages` array support
...
- [x] `generate_context` command updated with payload sniffing:
  ...
  // Else convert: system_prompt + user_message → messages array
...
- [x] `createTauriLanguageModel()` function implemented:
  ...
  // Bridges Vercel SDK → Tauri backend commands
...
- [x] Provider converts Vercel SDK format → backend `ContextPayload`
...
- [x] Vercel AI SDK tool schemas defined for:
  ...
  - `list_dir`: `{ path: string }` → returns file/folder list
  - `git_status`: `{}` → returns branch, uncommitted files
  - `git_log`: `{ limit?: number }` → returns recent commits
...
- [x] Expected: Model calls `list_dir` → `read_file(package.json)` → synthesizes answer
...
- [x] **3. Update OpenRouterProvider (src-tauri/src/ai/providers/openrouter.rs)**
  ...
  - [x] 3.4. Write integration test: Messages array → OpenRouter API call
...
- [x] **4. Update generate_context Command (src-tauri/src/commands/ai.rs)**
  ...
  - [x] 4.2. If `messages` missing, convert `system_prompt + user_message` → messages array
...
- [x] **6. Implement Vercel AI SDK Client (src/lib/ai/client.ts - NEW)**
  ...
  - [x] 6.6. Write unit test: Mock Tauri events → Vercel SDK stream
...
- Solution: Implement `LanguageModelV1` interface that bridges Vercel SDK → Tauri backend
...
1. ✅ Debug page shows successful tool calls in console
2. ✅ Reasoning store updates visible in React DevTools
3. ✅ All 3 test cases pass (single tool, multi-step, max steps)
4. ✅ Existing Story 3.4 ContextPanel functionality unaffected
...
```
Based on: ∞ 10 commits · 📄 5 files · 📝 DEVLOG
```

**Future Attribution (After Story 4.5.2):**
```
Based on: ∞ 10 commits · 📄 5 files · 📝 DEVLOG · 🔧 read_file×3 git_log×1
```
...
2. If response contains tool call → Execute tool → Add to messages → Call LLM again
3. Repeat until:
